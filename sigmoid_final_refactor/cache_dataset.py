...
from config import DATA_DIR, CACHE_DIR, NUM_FEATURE_CATEGORY_ID, MONTH_LIST
...
def main():
    ...
    train_valid_sessions = pd.read_csv(
        f'{DATA_DIR}/train_sessions.csv',
        parse_dates=['date']
    )

    train_valid_purchases = pd.read_csv(
        f'{DATA_DIR}/train_purchases.csv',
        parse_dates=['date']
    )
    ...
    for split_index, month in enumerate(tqdm(MONTH_LIST[: -1])):
        ...
        to_pickle(obj=grouped_sessions, path=f'{CACHE_DIR}/train_sessions_{split_index}.pkl')
        to_pickle(obj=impression_items, path=f'{CACHE_DIR}/train_impression_items_{split_index}.pkl')
        to_pickle(obj=candidate_items, path=f'{CACHE_DIR}/train_candidate_items_{split_index}.pkl')
        to_pickle(obj=grouped_sessions, path=f'{CACHE_DIR}/train_valid_sessions_{split_index}.pkl')
        to_pickle(obj=impression_items, path=f'{CACHE_DIR}/train_valid_impression_items_{split_index}.pkl')
        to_pickle(obj=candidate_items, path=f'{CACHE_DIR}/train_valid_candidate_items_{split_index}.pkl')

    assert len(train_session_ids) == len(train_grouped_sessions)
    to_pickle(obj=train_grouped_sessions, path=f'{CACHE_DIR}/train_sessions.pkl')
    ...
    to_pickle(obj=valid_grouped_sessions, path=f'{CACHE_DIR}/valid_sessions.pkl')
    to_pickle(obj=impression_items, path=f'{CACHE_DIR}/valid_impressvalid_candidate_itemsion_items.pkl')
    to_pickle(obj=candidate_items, path=f'{CACHE_DIR}/.pkl')
    to_pickle(obj=valid_grouped_sessions, path=f'{CACHE_DIR}/train_valid_sessions_{len(MONTH_LIST) - 1}.pkl')
    to_pickle(obj=impression_items, path=f'{CACHE_DIR}/train_valid_impression_items_{len(MONTH_LIST) - 1}.pkl')
    to_pickle(obj=candidate_items, path=f'{CACHE_DIR}/train_valid_candidate_items_{len(MONTH_LIST) - 1}.pkl')

    train_valid_grouped_sessions = train_grouped_sessions + valid_grouped_sessions
    to_pickle(obj=train_valid_grouped_sessions, path=f'{CACHE_DIR}/train_valid_sessions.pkl')
    ...
    train_item_features = pd.read_csv(f'{DATA_DIR}/item_features.csv')
    ...
    to_pickle(obj=train_feature_counts, path=f'{CACHE_DIR}/train_feature_counts.pkl')

    train_item_counts = OrderedDict(sorted(train_item_counts.items(), key=lambda item: item[1], reverse=True))
    to_pickle(obj=train_item_counts, path=f'{CACHE_DIR}/train_item_counts.pkl')

    train_valid_item_features = pd.read_csv(f'{DATA_DIR}/item_features.csv')
    ...
    to_pickle(obj=train_valid_feature_counts, path=f'{CACHE_DIR}/train_valid_feature_counts.pkl')

    train_valid_item_counts = OrderedDict(sorted(train_valid_item_counts.items(), key=lambda item: item[1], reverse=True))
    to_pickle(obj=train_valid_item_counts, path=f'{CACHE_DIR}/train_valid_item_counts.pkl')
    ...
    to_pickle(obj=item_features, path=f'{CACHE_DIR}/item_features.pkl')
