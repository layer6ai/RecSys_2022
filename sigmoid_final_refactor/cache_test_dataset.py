...
def main():
    test_leaderboard_sessions = pd.read_csv(
        f'{DATA_DIR}/test_leaderboard_sessions.csv',
        parse_dates=['date']
    )
    ...
    to_pickle(obj=test_leaderboard_session_ids, path=f'{CACHE_DIR}/test_leaderboard_session_ids.pkl')
    to_pickle(obj=test_leaderboard_grouped_sessions, path=f'{CACHE_DIR}/test_leaderboard_sessions.pkl')

    test_final_sessions = pd.read_csv(
        f'{DATA_DIR}/test_final_sessions.csv',
        parse_dates=['date']
    )
    ...
    to_pickle(obj=test_final_session_ids, path=f'{CACHE_DIR}/test_final_session_ids.pkl')
    to_pickle(obj=test_final_grouped_sessions, path=f'{CACHE_DIR}/test_final_sessions.pkl')

    test_candidate_items = pd.read_csv(f'{DATA_DIR}/candidate_items.csv')
    test_candidate_items = sorted(test_candidate_items['item_id'].unique())
    to_pickle(obj=test_candidate_items, path=f'{CACHE_DIR}/test_candidate_items.pkl')
